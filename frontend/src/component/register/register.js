    ...
    const handleRegister = async(e)=>{
        ...
        try{
         const response = await fetch('http://localhost:8080/signup',{
            ...
            body:JSON.stringify({name,email,password,"role_id":1})
         })
    ...
    return (
        <div class="flex">
            ...
            <div className="w-1/2 p-10">
                <h2 className="text-3xl font-bold mb-4 text-center justify-center">Register</h2>
                <form onSubmit={handleRegister}>
                    <div class='mb-4 flex items-center'>
                        <label class='w-1/3 text-2xl font-medium mr-4'>Name:</label>
                        <input value={name} onChange={(e)=>setName(e.target.value)} type="text" id="name" placeholder="Enter your name here" class='mt-1 p-2 w-full border rounded-xl focus:outline-none focus:ring focus:boder-based-300'></input>
                    </div>
                    <div class='mb-4 flex items-center' >
                ...
                <p className="mt-4 text-center font-poppins">
                    Already have an account ? <Link class="text-based font-extrabold" to='/'>Login</Link>
                </p>
            </div>
        </div>
    )
